...
const LS = {
  ...
  gpsLog: 'sinc_alarmas_gps_log_v1',
  gpsTrackingEnabled: 'sinc_alarmas_gps_tracking_enabled_v1'
};
...
const state = {
  ...
  ackUploads: {},
  gpsLog: loadGpsLog(),
  gpsTrackingEnabled: localStorage.getItem(LS.gpsTrackingEnabled) === 'true',
  gpsTimer: null,
  lastGpsMinuteKey: '',
...
function setupEvents(){
  ...
  safeOn('gpsStartBtn', 'click', activateGpsTracking);
  safeOn('gpsStopBtn', 'click', deactivateGpsTracking);
  safeOn('gpsClearBtn', 'click', clearGpsLog);
  safeOn('gpsRefreshBtn', 'click', () => captureGpsPosition(true));
...
function loadGpsLog(){
  try {
    const parsed = JSON.parse(localStorage.getItem(LS.gpsLog) || '[]');
    return Array.isArray(parsed) ? parsed : [];
  } catch {
    return [];
  }
...
function saveGpsLog(){
  const maxRecords = 1440; // conserva como máximo las últimas 24 horas si la app registra 1 posición por minuto
  state.gpsLog = (state.gpsLog || [])
    .filter(r => r && r.timestamp)
    .sort((a,b) => Number(b.timestamp || 0) - Number(a.timestamp || 0))
    .slice(0, maxRecords);
  localStorage.setItem(LS.gpsLog, JSON.stringify(state.gpsLog));
  renderGpsLog();
}

function initGpsLogger(){
  renderGpsLog();
  if(state.gpsTrackingEnabled) startGpsMinuteLogger();
}

function setGpsTrackingEnabled(enabled){
  state.gpsTrackingEnabled = Boolean(enabled);
  localStorage.setItem(LS.gpsTrackingEnabled, state.gpsTrackingEnabled ? 'true' : 'false');
  renderGpsLog();
}

function startGpsMinuteLogger(){
  if(state.gpsTimer) clearInterval(state.gpsTimer);
  captureGpsPosition(false);
  state.gpsTimer = setInterval(() => captureGpsPosition(false), 60000);
}

function activateGpsTracking(){
  setGpsTrackingEnabled(true);
  startGpsMinuteLogger();
}

function deactivateGpsTracking(){
  if(state.gpsTimer){
...
function clearGpsLog(){
  if(!confirm('¿Desea borrar todos los registros GPS?')) return;
  state.gpsLog = [];
  state.lastGpsMinuteKey = '';
  localStorage.setItem(LS.gpsLog, JSON.stringify(state.gpsLog));
  renderGpsLog();
}

function captureGpsPosition(manual){
  const lastTarget = $('gpsLastRecord');
  if(!manual && !state.gpsTrackingEnabled){
    renderGpsLog();
    return;
  }
  if(!navigator.geolocation){
    if(lastTarget) lastTarget.textContent = 'GPS no disponible en este dispositivo o navegador.';
    return;
  }
  ...
  if(!manual && state.lastGpsMinuteKey === minuteKey) return;

  if(lastTarget) lastTarget.textContent = manual ? 'Actualizando posición GPS...' : 'Registrando posición GPS...';

  navigator.geolocation.getCurrentPosition(
    pos => {
      state.lastGpsMinuteKey = minuteKey;
      ...
      state.gpsLog.unshift(record);
      saveGpsLog();
    },
    err => {
      const msg = err && err.code === 1
        ? 'Permiso de GPS denegado. Activa la ubicación para registrar posiciones minuto a minuto.'
        : 'No se pudo obtener la posición GPS. Revisa cobertura, permisos o conexión segura HTTPS.';
      if(lastTarget) lastTarget.textContent = msg;
      renderGpsLog();
    },
...
function renderGpsLog(){
  const lastTarget = $('gpsLastRecord');
  const listTarget = $('gpsLogList');
  const records = (state.gpsLog || []).slice().sort((a,b) => Number(b.timestamp || 0) - Number(a.timestamp || 0));
  const last = records[0];
  const startBtn = $('gpsStartBtn');
  const stopBtn = $('gpsStopBtn');
  if(startBtn) startBtn.disabled = state.gpsTrackingEnabled;
  if(stopBtn) stopBtn.disabled = !state.gpsTrackingEnabled;

  if(lastTarget){
    lastTarget.innerHTML = last
      ? '<strong>Último registro:</strong> ' + escapeHtml(last.date || '-') + ' · ' + escapeHtml(last.time || '-') + '<br><strong>Localización:</strong> ' + escapeHtml(formatGpsLocation(last))
      : (state.gpsTrackingEnabled ? 'Seguimiento GPS activado. Todavía no hay registros guardados.' : 'Seguimiento GPS desactivado. Pulsa ACTIVAR para iniciar el registro minuto a minuto.');
  }

  if(!listTarget) return;
  if(!records.length){
    listTarget.innerHTML = '<p class="hint">Pulsa ACTIVAR para registrar automáticamente una posición cada minuto. Pulsa ACTUALIZAR para guardar una posición puntual.</p>';
    return;
  }

  listTarget.innerHTML = records.map(r => (
    '<div class="gps-log-item">' +
...
function updateReceiverOverlayState(alarm){
  ...
  if(alarm.status === 'cancelled'){
    ...
    if(status) status.textContent = 'El otro móvil ha desactivado la alarma. Los registros GPS recibidos permanecen visibles en este teléfono.';
    ...
    if(status) status.textContent = 'Alarma activa. Recibiendo posición GPS del móvil emisor.';
...
function confirmReceived(){
  if(!state.ringingAlarm) return;
  stopTone();
  const current = state.ringingAlarm;
  state.ringingAlarm = null;
  stopReceiverGpsPolling();
  $('receiverOverlay').classList.add('hidden');

  // Si el emisor ya anuló/desactivó la alarma, el botón solo cierra el pop-up.
  // No se pisa el estado ANULADA ni se borra el historial GPS recibido.
...
function syncOnlineAlarms(remoteMap){
  ...
  for(const remoteAlarm of remoteAlarms){
    ...
    if(state.ringingAlarm && state.ringingAlarm.id === alarm.id && alarm.status !== 'pending'){
      // Si el emisor anula/desactiva la alarma, se para el sonido, pero NO se cierra
      // el pop-up del receptor ni se borran los GPS recibidos. El historial debe quedar
      // visible en el teléfono receptor mientras el pop exista.
      stopTone();
      state.ringingAlarm = alarm;
      updateReceiverOverlayState(alarm);
  ...
  if(state.ringingAlarm){
    const refreshed = state.alarms.find(a => a.id === state.ringingAlarm.id);
    if(refreshed){
      state.ringingAlarm = refreshed;
...
function attachSenderGpsLogToAlarm(alarmId, records, persistAndRender){
  ...
  const incoming = normalizeSenderGpsLog(records).filter(r => !r.from || r.from === alarm.from || r.from === state.deviceId);
  const byId = {};
