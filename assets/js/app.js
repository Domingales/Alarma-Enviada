...
const APP_VERSION = 'ALARMAS-LUCIA-v3.1-COMPROBACION-INMOVILIDAD-EMISOR-20260806';
...
async function requestSenderImmobilityCheck(alarm, durationMs, radiusM, maxDistance, latestLog, alertAt){
  ...
  const checkId = `${alarm.id}_${now}_${safeKey(state.deviceId)}`;
  const patch = {
    ...
    immobilityCheckDurationMs: Number(durationMs || getImmobilityDurationMs()),
    immobilityCheckRadiusM: Number(radiusM || getImmobilityRadiusM()),
    immobilityCheckMaxDistanceM: Number(maxDistance || 0),
    immobilityCheckLatestTimestamp: Number(latestLog?.timestamp || now),
    immobilityCheckDetectionAt: Number(alertAt || now)
  };
  ...
  try {
    await dbUpdateAlarm(alarm.id, patch);
    toast('Comprobación enviada al móvil emisor.');
  } catch(err){
    console.warn('No se pudo solicitar la comprobación al emisor:', err);
    alarm.immobilityCheckActive = false;
    alarm.immobilityCheckId = '';
    saveAlarms();
...
function showWellbeingResponsePopup(alarm){
  if(!$('wellbeingResponsePopup')?.classList.contains('hidden')) return;
  state.receiverResponseSaverSnapshot = suspendEnergySaverForCriticalPopup();
  state.wellbeingResponseAlarmId = String(alarm?.id || '');
  setText('wellbeingResponseAlarmText', alarm?.message ? `Alarma: ${alarm.message}` : 'Respuesta del emisor');
...
function closeWellbeingResponsePopup(){
  ...
  if(alarm){
    ...
    alarm.immobilityWellbeingNoticeAcknowledgedAt = Date.now();
    saveAlarms();
  }
  if(alarmId && state.firebaseReady){
    dbUpdateAlarm(alarmId, {
      immobilityWellbeingNoticeActive: false,
      immobilityWellbeingNoticeAcknowledgedAt: Date.now()
...
function showIncidentAlert(alarm){
  const alarmId = String(alarm?.id || '');
  if(state.incidentAlarmId === alarmId && !$('incidentAlertPopup')?.classList.contains('hidden')) return;
  stopIncidentAlarmEffects();
  state.incidentSaverSnapshot = suspendEnergySaverForCriticalPopup();
  state.incidentAlarmId = alarmId;
  setText('incidentAlarmText', alarm?.message ? `Alarma: ${alarm.message}` : 'El emisor ha indicado que no está bien.');
...
function silenceIncidentAlert(){
  ...
  if(alarm){
    ...
    alarm.immobilityIncidentSilencedAt = Date.now();
    saveAlarms();
  }
  if(alarmId && state.firebaseReady){
    dbUpdateAlarm(alarmId, {
      immobilityIncidentActive: false,
      immobilityIncidentSilencedAt: Date.now(),
