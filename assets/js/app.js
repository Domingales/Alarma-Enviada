...
const APP_VERSION = 'GPS-POP-v6-IPHONE-FORZADO-20260710';
...
function setupEvents(){
  ...
  safeOn('gpsPermissionBtn', 'click', requestGpsPermissionFromButton);
...
function updateStatusLine(){
  ...
  const mode = state.firebaseMode === 'rest' ? 'REST' : (state.firebaseMode === 'sdk' ? 'Firebase' : '');
  const msg = state.firebaseReady
    ? 'Sincronización online activa' + (mode ? ' (' + mode + ')' : '') + channel + ' · ' + APP_VERSION
...
async function sendAlarm(){
  ...
  let onlineOk = false;
  try{
    onlineOk = await pushAlarmOnline(alarm);
  }catch(err){
    console.error('No se pudo enviar la alarma online:', err);
    setText('sendInfo', 'La alarma se guardó en este móvil, pero NO se pudo escribir en Firebase. Revisa conexión/reglas de Firebase.');
  }

  startGpsUploadForAlarm(alarm);
  if(onlineOk){
    setText('sendInfo', gpsSendInfoText());
  } else if(!state.firebaseReady){
    setText('sendInfo', 'Alarma guardada en este móvil. Para que llegue al otro por internet debe aparecer Sincronización online activa.');
  }
  $('alarmNote').value = '';
  setDefaultDateTime();
  renderAlarms();
...
function normalizeAlarm(alarm){
  return {
    ...
    originalTo: String(alarm.originalTo || ''),
    acceptedByPairFallback: Boolean(alarm.acceptedByPairFallback),
...
function alarmCardHtml(a, compact){
  ...
  const recovered = a.acceptedByPairFallback ? ' · receptor corregido automáticamente' : '';
  const extra = compact ? '' : `<div class="small">Origen: ${escapeHtml(a.from || '-') } · Destino: ${escapeHtml(a.to || '-')}${escapeHtml(recovered)} · ID: ${escapeHtml(a.id)}</div>`;
...
function initFirebase(){
  ...
  if(state.db && state.firebaseListenersActive){
    try{ state.db.off(); }catch{}
    state.firebaseListenersActive = false;
  }
  stopAllGpsListeners();
  stopAllGpsUploads();

  if(!configured || !state.pairKey){
  ...
  // Modo preferente: SDK oficial de Firebase si el navegador lo carga bien.
  if(typeof firebase !== 'undefined'){
    try{
      ...
      state.db.on('value', snap => syncOnlineAlarms(snap.val() || {}), err => {
        console.error('Error leyendo Firebase SDK. Paso a modo REST:', err);
        startFirebaseRestMode();
      });
      ensureGpsRuntimeForAlarms();
      return;
    }catch(err){
      console.error('Firebase SDK no se pudo iniciar. Paso a modo REST:', err);
    }
  }

  // Fallback crítico para móviles/navegadores que no cargan gstatic/firebase.
  // Usa directamente la API REST de Realtime Database, sin depender del SDK externo.
  startFirebaseRestMode();
}

function startFirebaseRestMode(){
  const cfg = window.SINC_FIREBASE_CONFIG || {};
  if(!cfg.databaseURL || !state.pairKey || !('fetch' in window)){
    state.firebaseReady = false;
    state.firebaseMode = '';
    updateStatusLine();
    return;
...
function pushAlarmOnline(alarm){
  ...
  if(!state.firebaseReady) return Promise.resolve(false);
  if(state.firebaseMode === 'sdk' && state.db){
    return state.db.child(alarm.id).set(normalizeAlarm(alarm)).then(() => true).catch((err) => {
      console.error('No se pudo guardar en Firebase SDK. Paso a modo REST:', err);
      startFirebaseRestMode();
      return restSet('alarms/' + encodeURIComponent(alarm.id), normalizeAlarm(alarm)).then(() => true);
    });
  }
  if(state.firebaseMode === 'rest'){
    return restSet('alarms/' + encodeURIComponent(alarm.id), normalizeAlarm(alarm)).then(() => true);
  }
  return Promise.resolve(false);
}

function isRemoteAlarmRelevantToThisDevice(alarm){
  if(!alarm || !alarm.id) return false;
  if(alarm.from === state.deviceId || alarm.to === state.deviceId) return true;

  // V6: en este proyecto el canal Firebase se crea con una contraseña compartida
  // entre SOLO dos móviles. Por tanto, cualquier alarma del mismo canal que no
  // haya sido creada por este móvil debe tratarse como alarma entrante, aunque
  // el campo `to` conserve un DNI antiguo del receptor. Esto evita el fallo en
  // iPhone/Safari cuando el navegador cambia o limpia el DNI local.
  if(state.pairKey && alarm.from && alarm.from !== state.deviceId) return true;
  return false;
}

function adaptRemoteAlarmForThisDevice(alarm){
  const adapted = normalizeAlarm(alarm);
  if(adapted.from !== state.deviceId && adapted.to !== state.deviceId){
    adapted.originalTo = adapted.originalTo || adapted.to;
    adapted.to = state.deviceId;
    adapted.acceptedByPairFallback = true;
    adapted.acceptedByChannelFallback = true;
    adapted.updatedAt = Math.max(Number(adapted.updatedAt || 0), Number(adapted.createdAt || 0));
  }
  return adapted;
}

function syncOnlineAlarms(remoteMap){
  const remoteAlarms = Object.values(remoteMap || {})
    .map(normalizeAlarm)
    .filter(isRemoteAlarmRelevantToThisDevice)
    .map(adaptRemoteAlarmForThisDevice);
...
function autoConfirmProgrammed(alarm){
  ...
  if(programmed.acceptedByPairFallback || programmed.acceptedByChannelFallback || (programmed.originalTo && programmed.originalTo !== state.deviceId)){
    patch.to = state.deviceId;
    patch.originalTo = programmed.originalTo || '';
    patch.acceptedByPairFallback = true;
    patch.acceptedByChannelFallback = Boolean(programmed.acceptedByChannelFallback);
  }
  ...
  updateAlarmOnline(programmed.id, patch).then(() => {
...
function firebaseRestBaseUrl(){
  const cfg = window.SINC_FIREBASE_CONFIG || {};
  const base = String(cfg.databaseURL || '').replace(/\/+$/, '');
  if(!base || !state.pairKey) return '';
  return base + '/pairs/' + encodeURIComponent(state.pairKey);
}

function restUrl(path){
  const base = firebaseRestBaseUrl();
  const clean = String(path || '').replace(/^\/+|\/+$/g, '');
  return base ? base + (clean ? '/' + clean : '') + '.json' : '';
}

async function restRequest(path, method, body){
  const url = restUrl(path);
  if(!url) throw new Error('REST sin URL válida');
  const options = { method, headers: {'Content-Type':'application/json'} };
  if(method !== 'GET') options.body = JSON.stringify(body === undefined ? null : body);
  const res = await fetch(url, options);
  if(!res.ok){
    const text = await res.text().catch(() => '');
    throw new Error('Firebase REST ' + method + ' ' + res.status + ': ' + text);
  }
  return res.json().catch(() => null);
}

function restSet(path, value){
  if(state.firebaseMode !== 'rest') return Promise.resolve(null);
  return restRequest(path, 'PUT', value).catch(err => {
    console.error('No se pudo escribir en Firebase REST:', err);
    updateStatusLine();
    throw err;
  });
}

function restPatch(path, value){
  if(state.firebaseMode !== 'rest') return Promise.resolve(null);
  return restRequest(path, 'PATCH', value).catch(err => {
    console.error('No se pudo actualizar en Firebase REST:', err);
    throw err;
  });
}
...
function startRestPolling(){
  stopRestPolling();
  restPollNow();
  state.restPollTimer = setInterval(restPollNow, 2500);
}
...
function stopRestPolling(){
  ...
  state.restPollBusy = false;
}

async function restPollNow(){
  if(!state.firebaseReady || state.firebaseMode !== 'rest' || state.restPollBusy) return;
  state.restPollBusy = true;
  try{
    const data = await restRequest('', 'GET');
    syncOnlineAlarms((data && data.alarms) || {});
    syncOnlineGpsLogs((data && data.gpsLogs) || {});
  }catch(err){
    console.error('No se pudo leer Firebase REST:', err);
  }finally{
    state.restPollBusy = false;
  }
}

function updateAlarmOnline(alarmId, patch){
  if(!alarmId) return Promise.resolve(null);
  if(state.firebaseMode === 'sdk' && state.db){
    return state.db.child(alarmId).update(patch).catch(err => {
      console.error('No se pudo actualizar con SDK. Reintentando con REST:', err);
      startFirebaseRestMode();
      return restPatch('alarms/' + encodeURIComponent(alarmId), patch);
    });
  }
  if(state.firebaseMode === 'rest') return restPatch('alarms/' + encodeURIComponent(alarmId), patch);
  return Promise.resolve(null);
}

function uploadGpsLogOnline(alarmId, timestamp, record){
  if(!alarmId || !timestamp || !state.firebaseReady) return Promise.resolve(null);
  if(state.firebaseMode === 'sdk'){
    const ref = gpsAlarmRef(alarmId);
    if(!ref) return Promise.resolve(null);
    return ref.child(String(timestamp)).set(record).catch(err => {
      console.error('No se pudo subir GPS con SDK. Reintentando con REST:', err);
      startFirebaseRestMode();
      return restSet('gpsLogs/' + encodeURIComponent(alarmId) + '/' + encodeURIComponent(String(timestamp)), record);
    });
  }
  if(state.firebaseMode === 'rest'){
    return restSet('gpsLogs/' + encodeURIComponent(alarmId) + '/' + encodeURIComponent(String(timestamp)), record);
...
function syncOnlineGpsLogs(remoteGpsMap){
  if(!remoteGpsMap) return;
  Object.keys(remoteGpsMap || {}).forEach(alarmId => {
    const values = Object.values(remoteGpsMap[alarmId] || {}).map(normalizeGpsLog).filter(log => log.alarmId === alarmId);
    if(values.length){
      mergeGpsLogs(alarmId, values);
      renderGpsViews(alarmId);
    }
  });
  renderAlarms();
}
...
function gpsRootRef(){
  if(!state.firebaseReady || state.firebaseMode !== 'sdk' || !state.pairKey || typeof firebase === 'undefined') return null;
...
function refreshGpsPermissionState(){
  if(!('geolocation' in navigator)){
    state.gpsPermissionStatus = 'unavailable';
    updateGpsPermissionUI('Este navegador no ofrece acceso GPS.');
    return;
  }
  if(navigator.permissions && navigator.permissions.query){
    navigator.permissions.query({name:'geolocation'}).then(result => {
      state.gpsPermissionStatus = result.state || state.gpsPermissionStatus;
      updateGpsPermissionUI();
      result.onchange = () => {
        state.gpsPermissionStatus = result.state || state.gpsPermissionStatus;
        updateGpsPermissionUI();
      };
    }).catch(() => updateGpsPermissionUI());
  } else {
    updateGpsPermissionUI();
  }
}

function updateGpsPermissionUI(customMessage){
  const status = $('gpsPermissionStatus');
  if(!status) return;
  if(customMessage){
  ...
  if(state.gpsPermissionStatus === 'granted' && state.gpsLastPosition){
    const lat = Number(state.gpsLastPosition.coords.latitude).toFixed(6);
    const lon = Number(state.gpsLastPosition.coords.longitude).toFixed(6);
    status.textContent = 'GPS autorizado. Última posición: ' + lat + ', ' + lon;
    return;
  }
  if(state.gpsPermissionStatus === 'granted'){
    status.textContent = 'GPS autorizado. Al enviar la alarma se subirán coordenadas cada minuto.';
    return;
  }
  if(state.gpsPermissionStatus === 'denied'){
    status.textContent = 'GPS denegado. En iPhone/Android debes permitir la ubicación para esta web desde los ajustes del navegador o del sistema.';
    return;
  }
  if(state.gpsPermissionStatus === 'prompt'){
    status.textContent = 'GPS pendiente de autorización. Pulsa “Autorizar GPS de este móvil”.';
    return;
  }
  status.textContent = 'Pulsa “Autorizar GPS de este móvil” antes de enviar la alarma.';
}

async function requestGpsPermissionFromButton(){
  const btn = $('gpsPermissionBtn');
  state.gpsPermissionStatus = 'prompt';
  updateGpsPermissionUI('Solicitando permiso GPS al iPhone... Si aparece el aviso del sistema, pulsa Permitir.');
  setText('sendInfo', 'Solicitando permiso GPS al iPhone...');
  if(btn){
    btn.disabled = true;
  ...
  try{
    const pos = await requestGpsPosition({fromButton:true});
    state.gpsPermissionStatus = 'granted';
    state.gpsLastPosition = pos;
    state.gpsLastPositionAt = Date.now();
    updateGpsPermissionUI();
    setText('sendInfo', 'GPS autorizado correctamente. Ahora puedes enviar la alarma.');
  }catch(err){
    const msg = gpsPermissionErrorMessage(err);
    updateGpsPermissionUI(msg);
    setText('sendInfo', msg);
  }finally{
    if(btn){
      btn.disabled = false;
      btn.textContent = btn.dataset.originalText || 'Autorizar GPS de este móvil';
    }
  }
}
...
function requestGpsPosition(options){
  ...
  return new Promise((resolve, reject) => {
    if(!('geolocation' in navigator)){
      state.gpsPermissionStatus = 'unavailable';
      reject({code:0, message:'GPS no disponible'});
      return;
    }
    ...
    const fail = (err) => {
      if(finished) return;
      finished = true;
      reject(err || {code:0, message:'Error GPS desconocido'});
    };

    const ua = navigator.userAgent || '';
    const isIOS = /iPad|iPhone|iPod/.test(ua) || (navigator.platform === 'MacIntel' && navigator.maxTouchPoints > 1);

    // En iPhone es más fiable pedir primero una lectura normal.
    // Si se exige alta precisión desde el principio, a veces tarda mucho
    // y el usuario cree que el botón no hace nada.
    const firstOptions = {
      enableHighAccuracy: !isIOS && !opts.fromButton,
      timeout: opts.fromButton ? 12000 : 22000,
      maximumAge: opts.fromButton ? 0 : 30000
    };

    navigator.geolocation.getCurrentPosition(done, (err) => {
      // Segundo intento más permisivo, útil en interiores o con GPS frío.
      if(err && err.code === 3){
...
function gpsPermissionErrorMessage(err){
  if(err && err.code === 1){
    state.gpsPermissionStatus = 'denied';
    return 'GPS denegado. En el iPhone permite Ubicación para esta web y vuelve a pulsar “Autorizar GPS de este móvil”. Revisa: Ajustes > Privacidad y seguridad > Localización > Safari/Sitios web, Chrome o WhatsApp, según desde dónde abras la app.';
  }
  if(err && err.code === 2){
    return 'No se pudo obtener la posición GPS. Comprueba que la ubicación esté activada y que tengas cobertura.';
  }
  if(err && err.code === 3){
    return 'El iPhone no devolvió ubicación a tiempo. Comprueba que Localización esté activada, abre la app en Safari/Chrome normal, no dentro de WhatsApp, y vuelve a pulsar “Autorizar GPS de este móvil”.';
  }
  return 'No se pudo autorizar el GPS en este navegador.';
}

function gpsSendInfoText(){
  if(!('geolocation' in navigator)) return 'Alarma enviada online, pero este navegador no ofrece GPS.';
  if(state.gpsPermissionStatus === 'denied') return 'Alarma enviada online, pero el GPS está denegado. El receptor recibirá la alarma sin coordenadas hasta que actives la ubicación.';
  if(state.gpsPermissionStatus === 'granted') return 'Alarma enviada online. El GPS del emisor se enviará al receptor cada minuto mientras la alarma siga pendiente.';
  return 'Alarma enviada online. Si el navegador pide permiso de ubicación, acepta para enviar coordenadas al receptor.';
}
...
function uploadGpsPosition(alarm){
  ...
  if(!state.firebaseReady || !('geolocation' in navigator)) return;
  state.gpsUploadBusy[alarm.id] = true;
  requestGpsPosition().then(pos => {
    ...
    uploadGpsLogOnline(alarm.id, ts, record).catch(err => {
    ...
    updateGpsPermissionUI(msg);
    if(err && err.code === 1){
      setText('sendInfo', 'Alarma enviada, pero el permiso GPS está denegado. Usa el botón “Autorizar GPS de este móvil” o activa la ubicación en ajustes.');
      stopGpsUploadForAlarm(alarm.id);
    }
  });
}
