...
const APP_VERSION = 'ALARMAS-LUCIA-v3.2-COMPROBACION-INMOVILIDAD-DIRECTA-20260806';
...
const REST_WELLBEING_POLL_MS = 1000;
...
const state = {
  ...
  db: null,
  restPollTimer: null,
  restWellbeingPollTimer: null,
  restGpsPollTimers: {},
  sdkAlarmRef: null,
  sdkWellbeingRef: null,
  sdkGpsRefs: {},
  ...
  immobilityDialogKey: '',
  immobilityPresented: {},
  immobilityRequestPending: {},
  immobilityResponseProcessing: {},
  wellbeingChecks: {},
  wellbeingCheckProcessing: {},
  activeSenderWellbeingCheckId: '',
  activeSenderImmobilityCheckId: '',
  activeSenderImmobilityAlarmId: '',
  senderImmobilitySaverSnapshot: null,
  receiverResponseSaverSnapshot: null,
  wellbeingResponseAlarmId: '',
  wellbeingResponseCheckId: '',
  dismissedWellbeingChecks: {},
  incidentSaverSnapshot: null,
  incidentAlarmId: '',
  incidentCheckId: '',
  silencedIncidentChecks: {},
  incidentToneTimer: null,
  incidentVibrationTimer: null,
...
function setupEvents(){
  ...
  on('immobilitySenderYesBtn', 'click', () => answerSenderImmobilityCheck('yes'));
  on('immobilitySenderNoBtn', 'click', () => answerSenderImmobilityCheck('no'));
  on('closeWellbeingResponseBtn', 'click', closeWellbeingResponsePopup);
  on('silenceIncidentBtn', 'click', silenceIncidentAlert);
...
function restartFirebaseSubscription(){
  ...
  if(state.firebaseMode === 'sdk'){
    // Se escucha primero el canal de comprobaciones para recuperar cualquier
    // pregunta pendiente antes de reactivar el cálculo de inmovilidad.
    startSdkWellbeingListener();
    startSdkAlarmListener();
  } else {
    startRestWellbeingPolling();
    startRestAlarmPolling();
  }
}
function stopAllRemoteListeners(){
  if(state.sdkAlarmRef){ try { state.sdkAlarmRef.off(); } catch{} state.sdkAlarmRef = null; }
  if(state.sdkWellbeingRef){ try { state.sdkWellbeingRef.off(); } catch{} state.sdkWellbeingRef = null; }
  ...
  if(state.restWellbeingPollTimer){ clearInterval(state.restWellbeingPollTimer); state.restWellbeingPollTimer = null; }
...
function startSdkWellbeingListener(){
  if(!state.db || !state.pairKey) return;
  state.sdkWellbeingRef = state.db.ref(`pairs/${state.pairKey}/wellbeingChecks`);
  state.sdkWellbeingRef.on('value', snap => processRemoteWellbeingChecks(snap.val() || {}), err => {
    console.warn('No se pudo escuchar el canal de comprobación de bienestar:', err);
  });
}
...
function startRestWellbeingPolling(){
  pollRemoteWellbeingRest();
  state.restWellbeingPollTimer = setInterval(pollRemoteWellbeingRest, REST_WELLBEING_POLL_MS);
}
...
async function pollRemoteWellbeingRest(){
  if(!state.pairKey || state.firebaseMode !== 'rest') return;
  try {
    const data = await restGet(`pairs/${state.pairKey}/wellbeingChecks`);
    processRemoteWellbeingChecks(data || {});
  } catch(err){ console.warn('REST wellbeing polling failed:', err); }
}
...
async function dbSetWellbeingCheck(check){
  if(!state.firebaseReady || !state.pairKey) throw new Error('Firebase no está activo.');
  if(!check?.id) throw new Error('Comprobación sin identificador.');
  const path = `pairs/${state.pairKey}/wellbeingChecks/${safeKey(check.id)}`;
  if(state.firebaseMode === 'sdk') await state.db.ref(path).set(check);
  else await restPut(path, check);
}
async function dbUpdateWellbeingCheck(checkId, patch){
  if(!state.firebaseReady || !state.pairKey) throw new Error('Firebase no está activo.');
  if(!checkId) throw new Error('Comprobación sin identificador.');
  patch.updatedAt = Date.now();
  const path = `pairs/${state.pairKey}/wellbeingChecks/${safeKey(checkId)}`;
  if(state.firebaseMode === 'sdk') await state.db.ref(path).update(patch);
  else await restPatch(path, patch);
}


function processRemoteWellbeingChecks(remoteObj){
  const entries = Object.entries(remoteObj || {})
    .map(([key, value]) => value && typeof value === 'object'
      ? Object.assign({ id: String(value.id || key) }, value)
      : null)
    .filter(Boolean)
    .filter(check => !check.pairKey || check.pairKey === state.pairKey)
    .sort((a,b) => Number(a.createdAt || 0) - Number(b.createdAt || 0));

  for(const check of entries){
    if(!check.id) continue;
    const previous = state.wellbeingChecks[check.id] || {};
    const merged = Object.assign({}, previous, check);
    state.wellbeingChecks[check.id] = merged;

    if(merged.senderDeviceId === state.deviceId){
      maybeShowSenderWellbeingCheck(merged);
    }
    if(merged.receiverDeviceId === state.deviceId){
      maybeHandleReceiverWellbeingCheck(merged);
    }
  }
}

function findAlarmForWellbeingCheck(check, direction){
  if(!check?.alarmId) return null;
  return state.alarms.find(alarm =>
    alarm.id === check.alarmId && (!direction || alarm.direction === direction)
  ) || state.alarms.find(alarm => alarm.id === check.alarmId) || null;
}

function hasOpenWellbeingCheckForAlarm(alarmId){
  return Object.values(state.wellbeingChecks).some(check =>
    check &&
    check.alarmId === alarmId &&
    check.receiverDeviceId === state.deviceId &&
    ['pending','delivered','answered'].includes(String(check.status || 'pending')) &&
    !check.handledAt
  );
}

function notifySenderWellbeingCheck(check){
  try {
    if(document.visibilityState !== 'visible' && window.Notification && Notification.permission === 'granted'){
      const notice = new Notification('¿VA TODO BIEN?', {
        body: 'Se ha detectado falta de movimiento. Abre Alarmas Lucía para responder.',
        tag: `wellbeing-${check.id}`,
  ...
  } catch(err){ console.warn('No se pudo mostrar la notificación de comprobación:', err); }
}

function wakeSenderScreenForWellbeingCheck(check, alarm){
  const message = String(alarm?.message || 'Comprobación de seguridad');
  try { window.focus(); } catch {}
  requestScreenWakeLock(true).catch(() => {});

  // En la app Android, estas llamadas permiten que una versión del puente nativo
  // encienda y desbloquee visualmente la pantalla para mostrar la pregunta.
  const nativeCalls = [
    ['showWellbeingCheck', [String(check.id), String(check.alarmId || ''), message]],
    ['wakeScreenForAlert', [String(check.id), message]],
    ['wakeScreen', []],
    ['turnScreenOn', []]
  ];
  for(const [method, args] of nativeCalls){
    if(!hasAndroidBridgeMethod(method)) continue;
    try { AndroidBridge[method](...args); break; }
    catch(err){ console.warn(`No se pudo ejecutar AndroidBridge.${method}:`, err); }
  }

  if(state.settings.vibrationEnabled !== false){
    try { if(navigator.vibrate) navigator.vibrate([350,180,350]); } catch {}
  }
  notifySenderWellbeingCheck(check);
}

function maybeShowSenderWellbeingCheck(check){
  if(!check || check.senderDeviceId !== state.deviceId) return;
  const status = String(check.status || 'pending');
  if(!['pending','delivered'].includes(status) || check.response) return;
  if(state.activeSenderWellbeingCheckId === check.id) return;
  if(state.activeSenderWellbeingCheckId && state.activeSenderWellbeingCheckId !== check.id) return;

  const alarm = findAlarmForWellbeingCheck(check, 'outgoing');
  state.activeSenderWellbeingCheckId = String(check.id);
  state.activeSenderImmobilityCheckId = String(check.id);
  state.activeSenderImmobilityAlarmId = String(check.alarmId || '');
  state.senderImmobilitySaverSnapshot = suspendEnergySaverForCriticalPopup();

  wakeSenderScreenForWellbeingCheck(check, alarm);
  setText('immobilitySenderAlarmText', alarm?.message ? `Alarma: ${alarm.message}` : 'Comprobación de seguridad');
  const popup = $('immobilitySenderPopup');
  popup?.classList.remove('hidden');
  popup?.setAttribute('aria-hidden', 'false');
  setImmobilitySenderButtonsDisabled(false);
  setTimeout(() => $('immobilitySenderYesBtn')?.focus(), 0);

  if(status === 'pending'){
    const deliveredAt = Date.now();
    check.status = 'delivered';
    check.deliveredAt = deliveredAt;
    check.deliveredTo = state.deviceId;
    dbUpdateWellbeingCheck(check.id, {
      status: 'delivered',
      deliveredAt,
      deliveredTo: state.deviceId
    }).catch(err => console.warn('No se pudo confirmar la entrega de la pregunta:', err));
  }
}

async function answerDirectSenderWellbeingCheck(response){
  const checkId = state.activeSenderWellbeingCheckId;
  const check = state.wellbeingChecks[checkId];
  if(!check || (response !== 'yes' && response !== 'no')) return false;

  setImmobilitySenderButtonsDisabled(true);
  const respondedAt = Date.now();
  const patch = {
    status: 'answered',
    response,
    respondedAt,
    respondedBy: state.deviceId
  };

  try {
    await dbUpdateWellbeingCheck(checkId, patch);
    Object.assign(check, patch);

    $('immobilitySenderPopup')?.classList.add('hidden');
    $('immobilitySenderPopup')?.setAttribute('aria-hidden', 'true');

    const saverSnapshot = state.senderImmobilitySaverSnapshot;
    state.activeSenderWellbeingCheckId = '';
    state.activeSenderImmobilityCheckId = '';
    state.activeSenderImmobilityAlarmId = '';
    state.senderImmobilitySaverSnapshot = null;

    if(response === 'yes'){
      restoreEnergySaverAfterCriticalPopup(saverSnapshot);
      toast('Respuesta enviada: todo está bien.');
    } else {
      toast('Se ha enviado al receptor el aviso de posible incidencia.');
    }
    return true;
  } catch(err){
    console.warn('No se pudo enviar la respuesta de bienestar:', err);
    setImmobilitySenderButtonsDisabled(false);
    toast('No se pudo enviar la respuesta. Comprueba la conexión e inténtalo de nuevo.');
    return true;
  }
}

function maybeHandleReceiverWellbeingCheck(check){
  if(!check || check.receiverDeviceId !== state.deviceId) return;
  const response = String(check.response || '').toLowerCase();

  if(check.handledAt){
    if(check.noticeActive === true){
      const alarm = findAlarmForWellbeingCheck(check, 'incoming');
      if(response === 'yes' && !state.dismissedWellbeingChecks[check.id]){
        showWellbeingResponsePopup(alarm, check);
      } else if(response === 'no' && !state.silencedIncidentChecks[check.id]){
        showIncidentAlert(alarm, check);
      }
    }
    return;
  }

  if(String(check.status || '') !== 'answered' || !['yes','no'].includes(response)) return;
  if(state.wellbeingCheckProcessing[check.id]) return;
  state.wellbeingCheckProcessing[check.id] = true;

  const alarm = findAlarmForWellbeingCheck(check, 'incoming');
  const now = Date.now();
  const cooldownUntil = now + Number(check.durationMs || getImmobilityDurationMs());
  const alarmPatch = {
    immobilityTransport: 'wellbeingChecks-v2',
    immobilityCheckActive: false,
    immobilityCheckId: check.id,
    immobilityCheckRequestedAt: Number(check.createdAt || now),
    immobilityCheckRequestedBy: state.deviceId,
    immobilityCheckTargetDeviceId: check.senderDeviceId,
    immobilityCheckResponse: response,
    immobilityCheckRespondedAt: Number(check.respondedAt || now),
    immobilityCheckRespondedBy: check.respondedBy || check.senderDeviceId,
    immobilityCheckHandledAt: now,
    immobilityCheckHandledBy: state.deviceId,
    immobilityCheckCooldownUntil: cooldownUntil,
    immobilityCheckLastResolution: response,
    immobilityAlertActive: false,
    immobilityAlertAcknowledged: true,
    immobilityWellbeingNoticeActive: response === 'yes',
    immobilityWellbeingNoticeAt: response === 'yes' ? now : null,
    immobilityIncidentActive: response === 'no',
    immobilityIncidentStartedAt: response === 'no' ? now : null,
    immobilityIncidentSilencedAt: null
  };
  const checkPatch = {
    status: 'handled',
    handledAt: now,
    handledBy: state.deviceId,
    noticeType: response === 'yes' ? 'wellbeing' : 'incident',
    noticeActive: true
  };

  Object.assign(check, checkPatch);
  if(alarm){
    Object.assign(alarm, alarmPatch);
    state.webImmobilityState[alarm.id] = {
      active: false,
      firstAlertAt: 0,
      urgentShown: false,
    ...
    clearImmobilityPresentationForAlarm(alarm.id);
  ...
  if(response === 'yes') showWellbeingResponsePopup(alarm, check);
  else showIncidentAlert(alarm, check);

  const writes = [dbUpdateWellbeingCheck(check.id, checkPatch)];
  if(alarm) writes.push(dbUpdateAlarm(alarm.id, alarmPatch));
  Promise.allSettled(writes)
    .then(results => {
      results.forEach(result => {
        if(result.status === 'rejected') console.warn('No se pudo guardar por completo la respuesta de bienestar:', result.reason);
      });
    })
    .finally(() => { delete state.wellbeingCheckProcessing[check.id]; });
}
...
function processOutgoingRemoteUpdate(alarm){
  ...
  maybeShowSenderImmobilityCheck(alarm);
...
function processIncomingRemoteUpdate(alarm){
  ...
  maybeHandleReceiverImmobilityResponse(alarm);
  maybeRestorePendingImmobilityUi(alarm);
...
function suspendEnergySaverForCriticalPopup(){
  const snapshot = {
    wasActive: Boolean(state.energySaverActive),
    alarmId: String(state.energySaverAlarmId || '')
  };
  if(snapshot.wasActive) exitEnergySaverMode();
  return snapshot;
}

function restoreEnergySaverAfterCriticalPopup(snapshot){
  if(!snapshot?.wasActive || !snapshot.alarmId) return;
  const alarm = state.alarms.find(item => item.id === snapshot.alarmId);
  if(alarm && alarm.direction === 'outgoing' && isActiveAlarm(alarm)) enterEnergySaverMode(snapshot.alarmId);
}

function maybeShowSenderImmobilityCheck(alarm){
  if(alarm?.immobilityTransport === 'wellbeingChecks-v2') return;
  if(!alarm || alarm.from !== state.deviceId || alarm.direction === 'incoming') return;
  if(!alarm.immobilityCheckActive || alarm.immobilityCheckResponse) return;
  if(alarm.immobilityCheckTargetDeviceId && alarm.immobilityCheckTargetDeviceId !== state.deviceId) return;

  const checkId = String(alarm.immobilityCheckId || '');
  if(!checkId || state.activeSenderImmobilityCheckId === checkId) return;

  state.activeSenderImmobilityCheckId = checkId;
  state.activeSenderImmobilityAlarmId = String(alarm.id || '');
  state.senderImmobilitySaverSnapshot = suspendEnergySaverForCriticalPopup();
...
function setImmobilitySenderButtonsDisabled(disabled){
  const yes = $('immobilitySenderYesBtn');
  const no = $('immobilitySenderNoBtn');
  if(yes) yes.disabled = Boolean(disabled);
  if(no) no.disabled = Boolean(disabled);
}

async function answerSenderImmobilityCheck(response){
  if(response !== 'yes' && response !== 'no') return;
  if(state.activeSenderWellbeingCheckId){
    await answerDirectSenderWellbeingCheck(response);
    return;
  }
  const alarmId = state.activeSenderImmobilityAlarmId;
  const checkId = state.activeSenderImmobilityCheckId;
  const alarm = state.alarms.find(item => item.id === alarmId);
  if(!alarm || !checkId) return;

  setImmobilitySenderButtonsDisabled(true);
  const respondedAt = Date.now();
  const patch = {
    immobilityCheckResponse: response,
    immobilityCheckRespondedAt: respondedAt,
    immobilityCheckRespondedBy: state.deviceId
  };

  try {
    await dbUpdateAlarm(alarmId, patch);
    ...
    if(response === 'yes') restoreEnergySaverAfterCriticalPopup(saverSnapshot);
    else toast('Se ha enviado al receptor el aviso de posible incidencia.');
  } catch(err){
    console.warn('No se pudo enviar la respuesta de inmovilidad:', err);
    setImmobilitySenderButtonsDisabled(false);
    toast('No se pudo enviar la respuesta. Comprueba la conexión e inténtalo de nuevo.');
  }
}

async function requestSenderImmobilityCheck(alarm, durationMs, radiusM, maxDistance, latestLog, alertAt){
  if(!alarm || alarm.direction !== 'incoming' || !isActiveAlarm(alarm)) return;
  const now = Date.now();
  const cooldownUntil = Number(alarm.immobilityCheckCooldownUntil || 0);
  if(cooldownUntil > now) return;
  if(hasOpenWellbeingCheckForAlarm(alarm.id)) return;
  if(state.immobilityRequestPending[alarm.id]) return;

  const checkId = `WB_${safeKey(alarm.id)}_${now}_${safeKey(state.deviceId)}`;
  const check = {
    id: checkId,
    pairKey: state.pairKey,
    alarmId: alarm.id,
    senderDeviceId: alarm.from,
    receiverDeviceId: state.deviceId,
    status: 'pending',
    response: null,
    createdAt: now,
    detectionAt: Number(alertAt || now),
    durationMs: Number(durationMs || getImmobilityDurationMs()),
    radiusM: Number(radiusM || getImmobilityRadiusM()),
    maxDistanceM: Number(maxDistance || 0),
    latestTimestamp: Number(latestLog?.timestamp || now),
    alarmMessage: String(alarm.message || '')
  };
  const alarmPatch = {
    immobilityTransport: 'wellbeingChecks-v2',
    immobilityCheckActive: true,
    immobilityCheckId: checkId,
    immobilityCheckRequestedAt: now,
    immobilityCheckRequestedBy: state.deviceId,
    immobilityCheckTargetDeviceId: alarm.from,
    immobilityCheckResponse: null,
    immobilityCheckRespondedAt: null,
    immobilityCheckRespondedBy: null,
    immobilityCheckHandledAt: null,
    immobilityCheckDurationMs: check.durationMs,
    immobilityCheckRadiusM: check.radiusM,
    immobilityCheckMaxDistanceM: check.maxDistanceM,
    immobilityCheckLatestTimestamp: check.latestTimestamp,
    immobilityCheckDetectionAt: check.detectionAt
  };

  state.immobilityRequestPending[alarm.id] = checkId;
  state.wellbeingChecks[checkId] = check;

  try {
    // La solicitud dirigida al EMISOR se escribe primero en su canal específico.
    // El espejo en la alarma solo conserva compatibilidad y el control del monitor.
    await dbSetWellbeingCheck(check);
    Object.assign(alarm, alarmPatch);
    saveAlarms();
    nativeAcknowledgeImmobilityAlert(alarm.id);
    if(state.immobilityUrgentAlarmId === alarm.id) stopImmobilityTone();

    dbUpdateAlarm(alarm.id, alarmPatch)
      .catch(err => console.warn('La pregunta se entregó, pero no se pudo actualizar el espejo de la alarma:', err));
    toast('Comprobación enviada inmediatamente al móvil emisor.');
  } catch(err){
    console.warn('El canal directo de bienestar no respondió; se intenta el modo compatible:', err);
    delete state.wellbeingChecks[checkId];
    const legacyPatch = Object.assign({}, alarmPatch, { immobilityTransport: 'legacy-alarm-v1' });
    try {
      await dbUpdateAlarm(alarm.id, legacyPatch);
      Object.assign(alarm, legacyPatch);
      saveAlarms();
      nativeAcknowledgeImmobilityAlert(alarm.id);
      if(state.immobilityUrgentAlarmId === alarm.id) stopImmobilityTone();
      toast('Comprobación enviada al emisor mediante el canal compatible.');
    } catch(legacyErr){
      console.warn('No se pudo solicitar la comprobación al emisor:', legacyErr);
      toast('No se pudo contactar con el emisor. Comprueba la conexión.');
    }
  } finally {
    delete state.immobilityRequestPending[alarm.id];
  }
}

function maybeHandleReceiverImmobilityResponse(alarm){
  if(alarm?.immobilityTransport === 'wellbeingChecks-v2') return;
  if(!alarm || alarm.direction !== 'incoming' || !alarm.immobilityCheckId) return;
  if(alarm.immobilityCheckRequestedBy && alarm.immobilityCheckRequestedBy !== state.deviceId) return;
  const response = String(alarm.immobilityCheckResponse || '').toLowerCase();
  if(response !== 'yes' && response !== 'no') return;

  const respondedAt = Number(alarm.immobilityCheckRespondedAt || 0);
  const handledAt = Number(alarm.immobilityCheckHandledAt || 0);
  if(!respondedAt || handledAt >= respondedAt) return;
  const checkId = String(alarm.immobilityCheckId);
  if(state.immobilityResponseProcessing[checkId]) return;
  state.immobilityResponseProcessing[checkId] = true;

  const now = Date.now();
  const cooldownUntil = now + Number(alarm.immobilityCheckDurationMs || getImmobilityDurationMs());
  const patch = {
    immobilityCheckActive: false,
    immobilityCheckHandledAt: now,
    immobilityCheckHandledBy: state.deviceId,
  ...
  if(response === 'yes') showWellbeingResponsePopup(alarm);
  else showIncidentAlert(alarm);

  dbUpdateAlarm(alarm.id, patch)
    .catch(err => console.warn('No se pudo confirmar el tratamiento de la respuesta:', err))
    .finally(() => { delete state.immobilityResponseProcessing[checkId]; });
}

function clearImmobilityPresentationForAlarm(alarmId){
  Object.keys(state.immobilityPresented).forEach(key => {
    if(key.startsWith(`${alarmId}|`)) delete state.immobilityPresented[key];
  });
  const initialKey = `${alarmId}:initial`;
  delete state.immobilityShown[initialKey];
  saveImmobilityShown();
}

function showWellbeingResponsePopup(alarm, check=null){
  const checkId = String(check?.id || alarm?.immobilityCheckId || '');
  if(!$('wellbeingResponsePopup')?.classList.contains('hidden')){
    if(checkId && state.wellbeingResponseCheckId === checkId) return;
  }
  state.receiverResponseSaverSnapshot = suspendEnergySaverForCriticalPopup();
  state.wellbeingResponseAlarmId = String(alarm?.id || check?.alarmId || '');
  state.wellbeingResponseCheckId = checkId;
  setText('wellbeingResponseAlarmText', alarm?.message
    ? `Alarma: ${alarm.message}`
    : (check?.alarmMessage ? `Alarma: ${check.alarmMessage}` : 'Respuesta del emisor'));
  $('wellbeingResponsePopup')?.classList.remove('hidden');
  $('wellbeingResponsePopup')?.setAttribute('aria-hidden', 'false');
  setTimeout(() => $('closeWellbeingResponseBtn')?.focus(), 0);
}

function closeWellbeingResponsePopup(){
  $('wellbeingResponsePopup')?.classList.add('hidden');
  $('wellbeingResponsePopup')?.setAttribute('aria-hidden', 'true');
  const snapshot = state.receiverResponseSaverSnapshot;
  state.receiverResponseSaverSnapshot = null;
  const alarmId = state.wellbeingResponseAlarmId;
  const checkId = state.wellbeingResponseCheckId;
  state.wellbeingResponseAlarmId = '';
  state.wellbeingResponseCheckId = '';
  const acknowledgedAt = Date.now();
  const alarm = state.alarms.find(item => item.id === alarmId);
  if(checkId){
    state.dismissedWellbeingChecks[checkId] = true;
    const check = state.wellbeingChecks[checkId];
    if(check){
      check.noticeActive = false;
      check.noticeAcknowledgedAt = acknowledgedAt;
    }
    if(state.firebaseReady){
      dbUpdateWellbeingCheck(checkId, {
        noticeActive: false,
        noticeAcknowledgedAt: acknowledgedAt,
        noticeAcknowledgedBy: state.deviceId
      }).catch(err => console.warn('No se pudo guardar la confirmación del mensaje tranquilizador:', err));
    }
  }
  if(alarm){
    if(alarm.immobilityCheckId) state.dismissedWellbeingChecks[String(alarm.immobilityCheckId)] = true;
    alarm.immobilityWellbeingNoticeActive = false;
    alarm.immobilityWellbeingNoticeAcknowledgedAt = acknowledgedAt;
    saveAlarms();
  }
  if(alarmId && state.firebaseReady){
    dbUpdateAlarm(alarmId, {
      immobilityWellbeingNoticeActive: false,
      immobilityWellbeingNoticeAcknowledgedAt: acknowledgedAt
    }).catch(err => console.warn('No se pudo guardar la confirmación del aviso:', err));
  }
  restoreEnergySaverAfterCriticalPopup(snapshot);
}

function maybeRestorePendingImmobilityUi(alarm){
  if(!alarm || alarm.direction !== 'incoming') return;
  const checkId = String(alarm.immobilityCheckId || '');
  if(alarm.immobilityIncidentActive && !alarm.immobilityIncidentSilencedAt && !state.silencedIncidentChecks[checkId]){
    showIncidentAlert(alarm);
    return;
  }
  if(alarm.immobilityWellbeingNoticeActive && !state.dismissedWellbeingChecks[checkId]) showWellbeingResponsePopup(alarm);
}

function showIncidentAlert(alarm, check=null){
  const alarmId = String(alarm?.id || check?.alarmId || '');
  const checkId = String(check?.id || alarm?.immobilityCheckId || '');
  if(state.incidentCheckId === checkId && !$('incidentAlertPopup')?.classList.contains('hidden')) return;
  stopIncidentAlarmEffects();
  state.incidentSaverSnapshot = suspendEnergySaverForCriticalPopup();
  state.incidentAlarmId = alarmId;
  state.incidentCheckId = checkId;
  setText('incidentAlarmText', alarm?.message
    ? `Alarma: ${alarm.message}`
    : (check?.alarmMessage ? `Alarma: ${check.alarmMessage}` : 'El emisor ha indicado que no está bien.'));
  $('incidentAlertPopup')?.classList.remove('hidden');
  $('incidentAlertPopup')?.setAttribute('aria-hidden', 'false');
  startIncidentAlarmEffects();
  setTimeout(() => $('silenceIncidentBtn')?.focus(), 0);
}

function startIncidentAlarmEffects(){
  try {
    ...
    pattern();
    state.incidentToneTimer = setInterval(pattern, 1300);
  } catch(err){ console.warn('No se pudo iniciar el sonido de incidencia:', err); }

  const vibrate = () => {
    try { if(navigator.vibrate) navigator.vibrate([900, 300, 900]); } catch {}
  };
  vibrate();
  state.incidentVibrationTimer = setInterval(vibrate, 2400);
}

function stopIncidentAlarmEffects(){
  if(state.incidentToneTimer){
    clearInterval(state.incidentToneTimer);
    state.incidentToneTimer = null;
  }
  if(state.incidentVibrationTimer){
    clearInterval(state.incidentVibrationTimer);
    state.incidentVibrationTimer = null;
  }
  try { if(navigator.vibrate) navigator.vibrate(0); } catch {}
}

function silenceIncidentAlert(){
  stopIncidentAlarmEffects();
  $('incidentAlertPopup')?.classList.add('hidden');
  $('incidentAlertPopup')?.setAttribute('aria-hidden', 'true');
  const alarmId = state.incidentAlarmId;
  const checkId = state.incidentCheckId;
  state.incidentAlarmId = '';
  state.incidentCheckId = '';
  const silencedAt = Date.now();
  const alarm = state.alarms.find(item => item.id === alarmId);
  if(checkId){
    state.silencedIncidentChecks[checkId] = true;
    const check = state.wellbeingChecks[checkId];
    if(check){
      check.noticeActive = false;
      check.noticeSilencedAt = silencedAt;
    }
    if(state.firebaseReady){
      dbUpdateWellbeingCheck(checkId, {
        noticeActive: false,
        noticeSilencedAt: silencedAt,
        noticeSilencedBy: state.deviceId
      }).catch(err => console.warn('No se pudo guardar el silenciamiento de la incidencia:', err));
    }
  }
  if(alarm){
    if(alarm.immobilityCheckId) state.silencedIncidentChecks[String(alarm.immobilityCheckId)] = true;
    alarm.immobilityIncidentActive = false;
    alarm.immobilityIncidentSilencedAt = silencedAt;
    saveAlarms();
  }
  if(alarmId && state.firebaseReady){
    dbUpdateAlarm(alarmId, {
      immobilityIncidentActive: false,
      immobilityIncidentSilencedAt: silencedAt,
      immobilityIncidentSilencedBy: state.deviceId
    })
      .catch(err => console.warn('No se pudo guardar el silenciamiento:', err));
  }
  const snapshot = state.incidentSaverSnapshot;
  state.incidentSaverSnapshot = null;
  restoreEnergySaverAfterCriticalPopup(snapshot);
}
...
function nativeStartImmobilityMonitor(alarm){
  ...
  if(Number(alarm.immobilityCheckCooldownUntil || 0) > Date.now()){
    nativeStopImmobilityMonitor(alarm.id);
    return false;
  }
...
function evaluateImmobilityForAlarm(alarmId){
  ...
  const existingTracker = state.webImmobilityState[alarmId] || {};
  const suppressUntil = Math.max(
    Number(existingTracker.suppressUntil || 0),
    Number(alarm.immobilityCheckCooldownUntil || 0)
  );
...
async function showImmobilityWarning(alarm, durationMs, radiusM, maxDistance, latestLog, options={}){
  ...
  await requestSenderImmobilityCheck(
    alarm,
    Number(durationMs || getImmobilityDurationMs()),
    Number(radiusM || getImmobilityRadiusM()),
