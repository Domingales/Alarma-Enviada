...
const APP_VERSION = 'SINC-ALARMAS-NUEVA-v2-FIREBASE-FIJO-20260710';
...
function savePairing(){
  ...
  if(!/^MOVIL-[A-Z0-9]{4}-[A-Z0-9]{4}$/.test(peer)){
    setPanelInfo('pairInfo', 'Introduce un DNI con formato MOVIL-XXXX-XXXX.', true); return;
  }
...
function clearPairing(){
  if(!confirm('¿Borrar el emparejamiento de este móvil?')) return;
...
async function copyMyDeviceId(){
  try { await navigator.clipboard.writeText(state.deviceId); toast('DNI copiado.'); }
  catch { prompt('Copia este DNI:', state.deviceId); }
}
...
async function cancelAlarm(alarmId){
  ...
  if(!confirm('¿Anular esta alarma? El receptor verá que ha sido anulada.')) return;
...
function clearCompletedLocalHistory(){
  if(!confirm('¿Borrar del historial local las alarmas completadas o anuladas?')) return;
  const keep = [];
