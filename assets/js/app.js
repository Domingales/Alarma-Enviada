...
function upsertAlarm(alarm){
  ...
  if(i >= 0){
    ...
    // Protección contra el fallo detectado: nunca permitimos que una copia antigua
    // "pending" vuelva a activar una alarma que ya fue anulada o confirmada.
    if(isOlderAlarmUpdate(cleaned, existing)) return;
    state.alarms[i] = Object.assign({}, existing, cleaned);
  } else {
...
function isOlderAlarmUpdate(incoming, existing){
  ...
  // Si ya está anulada, jamás aceptamos una versión pendiente aunque venga sin fecha clara.
  if(existing.status === 'cancelled' && incoming.status === 'pending') return true;
  if(existing.status === 'received' && incoming.status === 'pending') return true;
  return false;
}
...
function renderPendingPreview(incomingPending, outgoingPending){
  ...
  if(!target) return;
  const pending = incomingPending.concat(outgoingPending).sort((a,b) => a.time - b.time);
  if(!pending.length){
    target.innerHTML = '<p class="hint">No hay alarmas pendientes.</p>';
    return;
  }
  target.innerHTML = pending.slice(0,5).map(a => alarmCardHtml(a, true)).join('');
}

function renderFullAlarmList(){
  const list = $('alarmList');
  if(!list) return;
  let alarms = state.alarms.slice();
...
function alarmCardHtml(a, compact){
  ...
  const cancelMsg = a.status === 'cancelled' && a.cancelMessage
    ? `<div class="cancel-message"><strong>Mensaje de anulación:</strong> ${escapeHtml(a.cancelMessage)}</div>`
    : '';
  // Solo puede anular la alarma el móvil que la envió.
  // El receptor la ve como pendiente, pero no debe poder cancelarla desde su lado.
  const canCancel = a.status === 'pending' && a.from === state.deviceId;
  const cancelBtn = canCancel
    ? `<div class="alarm-actions"><button type="button" class="cancel-alarm-btn" data-cancel-alarm="${escapeHtml(a.id)}">Anular alarma</button></div>`
    : '';
  const extra = compact ? '' : `<div class="small">Origen: ${escapeHtml(a.from || '-') } · Destino: ${escapeHtml(a.to || '-')}</div>`;
  return `<div class="alarm-item ${a.status}"><strong>${dir} · ${status}</strong><div>${when}</div><div>${note}</div>${cancelMsg}${extra}${cancelBtn}</div>`;
}

function handleAlarmActionClick(ev){
  const btn = ev.target.closest('[data-cancel-alarm]');
  if(!btn) return;
  cancelAlarm(btn.dataset.cancelAlarm);
}
...
function checkDueAlarms(){
  ...
  const due = state.alarms
    .filter(a => a.to === state.deviceId && a.status === 'pending' && a.time <= now)
...
function initFirebase(){
  ...
  try{
    ...
    if(state.db && state.firebaseListenersActive) state.db.off();
...
function pushAlarmOnline(alarm){
  if(state.firebaseReady && state.db){
    state.db.child(alarm.id).set(normalizeAlarm(alarm)).catch(() => {
      state.firebaseReady = false;
      updateStatusLine();
    });
  }
...
function receiveOnlineAlarm(alarm){
  if(!alarm || !alarm.id) return;
  const clean = normalizeAlarm(alarm);
  if(clean.from !== state.deviceId && clean.to !== state.deviceId) return;

  const before = state.alarms.find(a => a.id === clean.id);
  const wasPendingForMe = before && before.to === state.deviceId && before.status === 'pending';

  // Si el móvil receptor estaba sonando y llega una anulación/confirmación
  // desde Firebase, se corta inmediatamente y desaparece el botón RECIBIDO.
  if(state.ringingAlarm && state.ringingAlarm.id === clean.id && clean.status !== 'pending'){
    stopTone();
    state.ringingAlarm = null;
  ...
  // Caso crítico: el emisor anula antes de la hora. El receptor debe ver la anulación
  // automáticamente, con el mensaje, y no debe sonar nunca por esa alarma.
  if(clean.status === 'cancelled'){
    showCancellationNotice(clean, wasPendingForMe);
    return;
  }
