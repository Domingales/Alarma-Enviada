...
const APP_VERSION = 'GPS-POP-v5-RECEPTOR-AUTO-20260708';
...
function isRemoteAlarmRelevantToThisDevice(alarm){
  ...
  // Recuperación automática: si los dos móviles comparten canal y el emisor es
  // el móvil emparejado, se acepta la alarma aunque el campo `to` conserve un
  // DNI antiguo del receptor. Esto ocurre cuando un navegador limpia datos,
  // reinstala la PWA o cambia el DNI local después del emparejamiento.
  if(state.peerId && alarm.from === state.peerId && alarm.from !== state.deviceId) return true;
  return false;
}

function adaptRemoteAlarmForThisDevice(alarm){
  const adapted = normalizeAlarm(alarm);
  if(adapted.from !== state.deviceId && adapted.to !== state.deviceId && state.peerId && adapted.from === state.peerId){
...
function autoConfirmProgrammed(alarm){
  ...
  if(programmed.acceptedByPairFallback || (programmed.originalTo && programmed.originalTo !== state.deviceId)){
...
function requestGpsPermissionFromButton(){
  requestGpsPosition({fromButton:true}).then(pos => {
...
function gpsPermissionErrorMessage(err){
  if(err && err.code === 1){
    ...
    return 'GPS denegado. Permite la ubicación para esta web y vuelve a pulsar “Autorizar GPS de este móvil”. En iPhone suele estar en Ajustes > Privacidad y seguridad > Localización > Safari/Sitios web.';
  }
  if(err && err.code === 2){
    return 'No se pudo obtener la posición GPS. Comprueba que la ubicación esté activada y que tengas cobertura.';
  }
  if(err && err.code === 3){
    return 'El GPS tardó demasiado en responder. Acércate a una zona con mejor señal y vuelve a intentarlo.';
  }
