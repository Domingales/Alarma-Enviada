...
function alarmCardHtml(a, compact){
  ...
  const programmedMsg = a.from === state.deviceId && a.receiverProgrammedAt
    ? `<div class="programmed-message"><span class="green-check">✅</span> <strong>Recibida y programada en el otro móvil:</strong> ${escapeHtml(new Date(a.receiverProgrammedAt).toLocaleString('es-ES'))}</div>`
...
function syncOnlineAlarms(remoteMap){
  ...
  for(const alarm of remoteAlarms){
...
function autoConfirmProgrammed(alarm){
  const programmed = Object.assign({}, alarm, {
    receiverProgrammedAt: Date.now(),
    receiverProgrammedBy: state.deviceId,
    updatedAt: Math.max(Date.now(), Number(alarm.updatedAt || 0))
  });
  upsertAlarm(programmed);
  pushAlarmOnline(programmed);
}
