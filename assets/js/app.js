...
function loadSettings(){
  try {
    return Object.assign({theme:'blue', mode:'3d', sound:'tone1'}, JSON.parse(localStorage.getItem(LS.settings) || '{}'));
  } catch {
    return {theme:'blue', mode:'3d', sound:'tone1'};
  }
}
...
function normalizeAlarm(alarm){
  return {
    ...
    status: alarm.status === 'received' ? 'received' : 'pending',
...
function alarmCardHtml(a, compact){
  ...
  return `<div class="alarm-item ${a.status}"><strong>${dir} · ${status}</strong><div>${when}</div><div>${note}</div>${extra}</div>`;
}
