...
const LS = {
  ...
  gpsDeleted: 'sinc_alarmas_gps_deleted_ids_v1'
};
...
function setupEvents(){
  ...
  safeOn('cancelAlarmSendBtn', 'click', sendCancelAlarmMessage);
  safeOn('cancelAlarmCloseBtn', 'click', closeCancelAlarmPopup);
...
function loadGpsDeleted(){
  try { return JSON.parse(localStorage.getItem(LS.gpsDeleted) || '{}') || {}; }
  catch { return {}; }
}
function saveGpsDeleted(){ localStorage.setItem(LS.gpsDeleted, JSON.stringify(state.gpsDeleted || {})); }
function isGpsRecordDeleted(alarmId, recordId){
  return Boolean(state.gpsDeleted && state.gpsDeleted[String(alarmId)] && state.gpsDeleted[String(alarmId)][String(recordId)]);
}
function markGpsRecordDeleted(alarmId, recordId){
  const aid = String(alarmId || '');
  const rid = String(recordId || '');
  if(!aid || !rid) return;
  if(!state.gpsDeleted[aid]) state.gpsDeleted[aid] = {};
  state.gpsDeleted[aid][rid] = Date.now();
  saveGpsDeleted();
}
function markAllGpsRecordsDeleted(alarmId, records){
  const aid = String(alarmId || '');
  if(!aid) return;
  if(!state.gpsDeleted[aid]) state.gpsDeleted[aid] = {};
  normalizeSenderGpsLog(records).forEach(r => { state.gpsDeleted[aid][r.id] = Date.now(); });
  saveGpsDeleted();
}
...
function alarmCardHtml(a, compact){
  ...
  const programmedMsg = a.from === state.deviceId && ackAt
    ...
    : (a.from === state.deviceId && a.status === 'pending'
      ...
      : '');
  const latestSenderGps = getVisibleSenderGpsLog(a)[0] || null;
...
function handleAlarmActionClick(ev){
  ...
  if(deleteBtn){ deleteLocalRecord(deleteBtn.dataset.deleteRecord); return; }
  const deleteGpsBtn = ev.target.closest('[data-delete-gps]');
  if(deleteGpsBtn){ deleteReceiverGpsRecord(deleteGpsBtn.dataset.deleteGps); return; }
  const deleteAllGpsBtn = ev.target.closest('[data-delete-all-gps]');
  if(deleteAllGpsBtn){ deleteAllReceiverGpsRecords(deleteAllGpsBtn.dataset.deleteAllGps); return; }
}

function cancelAlarm(id){
  const alarm = state.alarms.find(a => a.id === id);
  if(!alarm || alarm.status !== 'pending' || alarm.from !== state.deviceId) return;
  state.cancelPendingAlarmId = id;
  const textarea = $('cancelAlarmMessage');
  if(textarea) textarea.value = '';
  const pop = $('cancelAlarmPopup');
  if(pop) pop.classList.remove('hidden');
}

function closeCancelAlarmPopup(){
  state.cancelPendingAlarmId = '';
  const pop = $('cancelAlarmPopup');
  if(pop) pop.classList.add('hidden');
}

function sendCancelAlarmMessage(){
  const id = state.cancelPendingAlarmId;
  const alarm = state.alarms.find(a => a.id === id);
  if(!alarm || alarm.status !== 'pending' || alarm.from !== state.deviceId){
    closeCancelAlarmPopup();
    return;
  }
  const message = ($('cancelAlarmMessage') && $('cancelAlarmMessage').value ? $('cancelAlarmMessage').value : '').trim();
  ...
  setText('sendInfo', 'Alarma anulada. El mensaje de anulación se ha enviado al otro móvil.');
}
...
function getVisibleSenderGpsLog(alarm){
  return normalizeSenderGpsLog(alarm && alarm.senderGpsLog)
    .filter(r => !isGpsRecordDeleted(alarm.id, r.id));
}

function renderReceiverSenderGps(alarm){
  const lastTarget = $('receiverSenderGpsLast');
  const listTarget = $('receiverSenderGpsList');
  if(!lastTarget || !listTarget || !alarm) return;
  const records = getVisibleSenderGpsLog(alarm);
  if(!records.length){
    lastTarget.innerHTML = 'Esperando posición GPS del móvil que envió la alarma...';
    listTarget.innerHTML = '<p class="hint">Cuando el móvil emisor tenga permiso de ubicación y conexión, aquí aparecerán sus registros minuto a minuto.</p>';
    return;
  }
  ...
  const deleteAll = '<button type="button" class="delete-all-gps-btn" data-delete-all-gps="' + escapeHtml(alarm.id) + '">BORRAR TODOS LOS REGISTROS</button>';
  const rows = records.map(r => (
    '<div class="receiver-gps-item">' +
      '<div class="receiver-gps-item-text">' +
        '<strong>' + escapeHtml(r.date || '-') + ' · ' + escapeHtml(r.time || '-') + '</strong>' +
        '<span>' + escapeHtml(formatGpsLocation(r)) + '</span>' +
      '</div>' +
      '<button type="button" class="delete-gps-btn" data-delete-gps="' + escapeHtml(alarm.id + '|' + r.id) + '">BORRAR</button>' +
    '</div>'
  )).join('');
  listTarget.innerHTML = '<div class="receiver-gps-toolbar">' + deleteAll + '</div>' + rows;
}

function deleteReceiverGpsRecord(payload){
  const parts = String(payload || '').split('|');
  const alarmId = parts[0];
  const recordId = parts.slice(1).join('|');
  if(!alarmId || !recordId) return;
  markGpsRecordDeleted(alarmId, recordId);
  const alarm = state.alarms.find(a => a.id === alarmId);
  if(alarm) renderReceiverSenderGps(alarm);
}

function deleteAllReceiverGpsRecords(alarmId){
  const alarm = state.alarms.find(a => a.id === alarmId);
  if(!alarm) return;
  if(!confirm('¿Borrar todos los registros GPS visibles de esta alarma en este móvil?')) return;
  markAllGpsRecordsDeleted(alarmId, alarm.senderGpsLog);
...
function updateReceiverOverlayState(alarm){
  ...
  const cancelBox = $('receiverCancelMessage');
  if(cancelBox){
    if(alarm && alarm.status === 'cancelled'){
      cancelBox.classList.remove('hidden');
      cancelBox.innerHTML = '<strong>Mensaje del emisor:</strong><br>' + escapeHtml(alarm.cancelMessage || 'Sin mensaje adicional.');
    } else {
...
function syncOnlineAlarms(remoteMap){
  ...
  for(const remoteAlarm of remoteAlarms){
    ...
    if(alarm.status === 'cancelled' && beforeStatus !== 'cancelled'){
      ...
      if(alarm.to === state.deviceId){
        const localCancelled = state.alarms.find(a => a.id === alarm.id) || alarm;
        showReceiverOverlay(localCancelled, 'tracking');
      } else {
